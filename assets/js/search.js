    ...
    try {
        ...
        gridEl.innerHTML = filtered.map(p => {
            ...
            if (currentStatus === 'Còn hàng') {
                buttonHtml = `<button class="add-to-cart search-add-btn" data-product-id="${safeId}" data-name="${p.name}" data-price="${p.price}" data-img="${safeImg}">Thêm vào giỏ hàng</button>`;
            } 
